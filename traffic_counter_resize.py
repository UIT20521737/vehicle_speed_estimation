...
url = './dataset/subset01/video02/video.h264'
...
width_min = 80 #Minimum rectangle width (largura_min=80)
height_min = 80 #Minimum rectangle height (altura_min=80)

framenumber = 0  # keeps track of current frames
...
ret, frame_org = cap.read()  # import image
ratio = .3  # resize ratio
frame_resized = cv2.resize(frame_org, (0, 0), None, ratio, ratio)  # resize image
width2, height2, channels = frame_resized.shape
...
while True:
    ...
    if ret:  # if there is a frame continue with code
        ...
        gray = cv2.cvtColor(frame_resized, cv2.COLOR_BGR2GRAY)  # converts image to gray
        blur = cv2.GaussianBlur(gray,(3,3),0)
        fgmask = fgbg.apply(blur)  # uses the background subtraction
        ...
        #=========  PRE PROCESSING VER 1  ========
        # kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (5, 5))  # kernel to apply to the morphology
        # closing = cv2.morphologyEx(fgmask, cv2.MORPH_CLOSE, kernel)
        # opening = cv2.morphologyEx(closing, cv2.MORPH_OPEN, kernel)
        # dilation = cv2.dilate(opening, kernel)
        # retvalbin, bins = cv2.threshold(dilation, 220, 255, cv2.THRESH_BINARY)  # removes the shadows
        #========= PRE PROCESSING VER 2 ==========
        # grey = cv2.cvtColor(frame,cv2.COLOR_BGR2GRAY)
        # blur = cv2.GaussianBlur(grey,(3,3),0)
        img_sub = fgbg.apply(blur)
        ret2,th2 = cv2.threshold(img_sub,0,255,cv2.THRESH_BINARY+cv2.THRESH_OTSU)
        dilation = cv2.dilate(th2,np.ones((15,2)))
        bins = cv2.morphologyEx (dilation, cv2.MORPH_CLOSE , kernel)
        bins = cv2.erode(bins,np.ones((5,5)))
        # bins = cv2.morphologyEx (closing, cv2.MORPH_CLOSE , kernel)
        ...
        for i in range(len(contours)):  # cycles through all contours in current frame
            ...
            if hierarchy[0, i, 3] == -1:  # using hierarchy to only count parent contours (contours not within others)
                ...
                if (w >= width_min and w < 200) and (h >= height_min and h < 200):  # area threshold for contour
                # if (minarea < area < maxarea):
                    ...
                    if cy > lineypos:  # filters out contours that are above line (y starts at top)
                        ...
                        car_detected = frame_resized[y:y + h, x:x + w]
        ...
        if len(cxx):  # if there are centroids in the specified area
            ...
            if not carids:  # if carids is empty
                ...
                for j in range(len(carids)):  # loops through all current car ids
                    ...
                    mindx = round(float(dx[minx_index, j]), 10)
                    mindy = round(float(dy[miny_index, j]), 10)
                    # print("mindx", mindx)
                    # print("mindy", mindy)
                    ...
                    elif np.abs(mindx) > maxrad or np.abs(mindy) > maxrad:
                        continue
        ...
        for i in range(currentcars):  # loops through all current car ids on screen
            ...
            if curcent:  # if there is a current centroid
                ...
                cv2.putText(frame_resized, "ID:" + str(carids[currentcarsindex[i]]), (int(curcent[0]), int(curcent[1] - 15)),
                            cv2.FONT_HERSHEY_SIMPLEX, .5, (0, 255, 255), 2)
                cv2.imwrite('CarDetected/car_{}.jpg'.format(str(carids[currentcarsindex[i]])), car_detected)
                # print ("ID: ", str(carids[currentcarsindex[i]]))
                # print ("================")
                # cv2.drawMarker(frame, (int(curcent[0]), int(curcent[1])), (0, 0, 255), cv2.MARKER_STAR, markerSize=5, thickness=1, line_type=cv2.LINE_AA)
        ...
        # cv2.putText(frame, "Cars in Area: " + str(currentcars), (0, 15), cv2.FONT_HERSHEY_SIMPLEX, .5, (0, 170, 0), 1)

        # cv2.putText(frame, "Cars Crossed Up: " + str(carscrossedup), (0, 30), cv2.FONT_HERSHEY_SIMPLEX, .5, (0, 170, 0),
        #             1)
        ...
        cv2.imshow("countours", frame_resized)
        ...
        # cv2.moveWindow("thres", 0, int(height * ratio))
